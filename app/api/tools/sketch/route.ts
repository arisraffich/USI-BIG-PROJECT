...
function normalizeModel(value: FormDataEntryValue | null): SketchModel {
    return value === 'nb-pro' || value === 'gpt-2' ? value : 'nb2'
}

function geminiModelId(model: SketchModel): string {
...
async function normalizeInputForReference(inputBuffer: Buffer): Promise<{ buffer: Buffer, width: number, height: number }> {
...
async function generateGeminiSketch(inputBuffer: Buffer, model: SketchModel): Promise<Buffer> {
    if (!GEMINI_API_KEY) throw new Error('Google API Key missing')

    const source = await normalizeInputForReference(inputBuffer)
    const payload = {
        contents: [{
            parts: [
                { text: SKETCH_PROMPT },
...
async function generateGpt2Sketch(inputBuffer: Buffer): Promise<Buffer> {
    if (!openai) throw new Error('OpenAI API Key not configured')

    const source = await normalizeInputForReference(inputBuffer)
    const imageUrl = `data:image/jpeg;base64,${source.buffer.toString('base64')}`
    const size = fitGpt2SizeToInputRatio(source.width, source.height)
    const content: OpenAIContentPart[] = [
        { type: 'input_text', text: 'SOURCE ILLUSTRATION TO CONVERT INTO A PENCIL SKETCH:' },
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(`[Create Sketch Tool] model=${model} file="${file.name}"`)

        const imageBuffer = model === 'gpt-2'
            ? await generateGpt2Sketch(inputBuffer)
            : await generateGeminiSketch(inputBuffer, model)
        ...
        return NextResponse.json({
            ...
            model,
            image: {
                dataUrl: await toJpegDataUrl(imageBuffer),
            },
        })
    } catch (error: unknown) {
        console.error('[Create Sketch Tool] error:', error)
