...
export function CoverDesignModal({ open, onOpenChange }: CoverDesignModalProps) {
    ...
    const [includeLineArt, setIncludeLineArt] = useState(false)
    ...
    }, [backCoverDataUrl, generateLineArt, includeLineArt, isFrontPicked, selectedCandidate, title])
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto space-y-5 pr-1">
                    <div className={candidates.length === 0 ? "grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-5" : "grid grid-cols-1 gap-5"}>
                        {candidates.length === 0 && <div className="space-y-4">
                            <div
                                onDrop={handleDrop}
                            ...
                            <Button onClick={handleGenerateFront} disabled={!canGenerate} className="w-full bg-purple-600 hover:bg-purple-700 text-white">
                                ...
                                {isGeneratingFront ? 'Generating 2 options...' : 'Generate Cover Options'}
                            </Button>
                        ...
                        <div className="space-y-4">
                            {candidates.length === 0 ? (
                                <div className="h-full min-h-[420px] rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-400">
                                    <div className="text-center">
                                        <BookImage className="w-10 h-10 mx-auto mb-3" />
                                        <p className="text-sm font-medium">Cover options will appear here</p>
                                    </div>
                                </div>
                            ) : (
                                <>
                                    {!isFrontPicked ? (
                                        ...
                                        <div className="space-y-4">
                                                ...
                                                <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
                                                    <div className="px-3 py-2 flex items-center justify-between border-b border-slate-100">
                                                        ...
                                                        <Button variant="outline" size="sm" onClick={handleGenerateBack} disabled={isBusy}>
                                                            {isGeneratingBack && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
                                            ...
                                            <div className="rounded-lg border border-slate-200 bg-white p-4 flex flex-col md:flex-row md:items-center gap-3 md:justify-between">
                                                <label className="flex items-start gap-2 text-sm text-slate-700">
                                                    <input
                                                        type="checkbox"
                                                        checked={includeLineArt}
                                                        onChange={(event) => setIncludeLineArt(event.target.checked)}
                                                        disabled={isBusy}
                                                        className="mt-0.5"
                                                    />
